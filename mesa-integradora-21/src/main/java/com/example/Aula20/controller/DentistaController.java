...
import com.example.Aula20.service.impl.DentistaServiceImpl;
...
public class DentistaController {
    ...
    @GetMapping
    public Map<Integer, Dentista> buscarTodos() {return dentistaService.buscarTodos();}
    ...
    public ResponseEntity<String> deletar(@PathVariable Integer id) {
        ...
        if (dentistaService.buscarPorId(id) != null) {
            ...
            return response = ResponseEntity.status(HttpStatus.NO_CONTENT).body("Dentista removido com sucesso");
        }
        response = ResponseEntity.status(HttpStatus.NOT_FOUND).body("Dentista não localizado");
        return response;
    }
    @PutMapping
    public ResponseEntity<Dentista> atualizar(@RequestBody Dentista dentista){
        ...
        if(dentista.getId() != null && dentista.getPaciente().getId() != null){
